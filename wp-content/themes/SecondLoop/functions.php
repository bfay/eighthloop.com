<?php
function secondloop_setup() {
	// Add language supports. Please note that secondloop Framework does not include language files.
	load_theme_textdomain('secondloop', get_template_directory() . '/lang');
	...
	register_nav_menus(array(
		'primary_navigation' => __('Primary Navigation', 'secondloop'),
		'utility_navigation' => __('Utility Navigation', 'secondloop')
	));	
}
add_action('after_setup_theme', 'secondloop_setup');

// Enqueue for header and footer, thanks to flickapix on Github.
// Enqueue css files
function secondloop_css() {
  if ( !is_admin() ) {
...
add_action( 'init', 'secondloop_css' );

function secondloop_ie_css () {
    echo '<!--[if lt IE 9]>';
    echo '<link rel="stylesheet" href="<?php echo get_template_directory_uri(); ?>/css/ie.css">';
    echo '<![endif]-->';
}
add_action( 'wp_head', 'secondloop_ie_css' );

// Enqueue js files
function secondloop_scripts() {
...
add_action( 'init', 'secondloop_scripts' );
...
function secondloop_entry_meta() {
	echo '<time class="updated" datetime="'. get_the_time('c') .'" pubdate>'. sprintf(__('Posted on %s at %s.', 'secondloop'), get_the_time('l, F jS, Y'), get_the_time()) .'</time>';
	echo '<p class="byline author vcard">'. __('Written by', 'secondloop') .' <a href="'. get_author_posts_url(get_the_author_meta('ID')) .'" rel="author" class="fn">'. get_the_author() .'</a></p>';
}
...
class secondloop_walker extends Walker_Nav_Menu {
...
function secondloop_active_nav_class( $classes, $item )
{
    if($item->current == 1)
    {
        $classes[] = 'active';
...
add_filter( 'nav_menu_css_class', 'secondloop_active_nav_class', 10, 2 );
...
// Pagination
function secondloop_pagination() {
	global $wp_query;
	...
	if ( $paginate_links ) {
		echo '<div class="secondloop-pagination">';
		echo $paginate_links;
