<?php
function eighthloop_setup() {
	// Add language supports. Please note that eighthloop Framework does not include language files.
	load_theme_textdomain('eighthloop', get_template_directory() . '/lang');
	...
	register_nav_menus(array(
		'primary_navigation' => __('Primary Navigation', 'eighthloop'),
		'utility_navigation' => __('Utility Navigation', 'eighthloop')
	));	
}
add_action('after_setup_theme', 'eighthloop_setup');
...
add_action( 'init', 'eighthloop_css' );
...
add_action( 'init', 'eighthloop_scripts' );
...
function eighthloop_entry_meta() {
	echo '<time class="updated" datetime="'. get_the_time('c') .'" pubdate>'. sprintf(__('Posted on %s at %s.', 'eighthloop'), get_the_time('l, F jS, Y'), get_the_time()) .'</time>';
	echo '<p class="byline author vcard">'. __('Written by', 'eighthloop') .' <a href="'. get_author_posts_url(get_the_author_meta('ID')) .'" rel="author" class="fn">'. get_the_author() .'</a></p>';
}
...
class eighthloop_walker extends Walker_Nav_Menu {
...
function eighthloop_active_nav_class( $classes, $item )
{
    if($item->current == 1)
    {
        $classes[] = 'active';
...
add_filter( 'nav_menu_css_class', 'eighthloop_active_nav_class', 10, 2 );
...
// Pagination
function eighthloop_pagination() {
	global $wp_query;
	...
	if ( $paginate_links ) {
		echo '<div class="eighthloop-pagination">';
		echo $paginate_links;
