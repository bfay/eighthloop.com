...
function fibonacci_setup() {
	// Add language supports. Please note that fibonacci Framework does not include language files.
	load_theme_textdomain('fibonacci', get_template_directory() . '/lang');
	...
	register_nav_menus(array(
		'primary_navigation' => __('Primary Navigation', 'fibonacci'),
		'utility_navigation' => __('Utility Navigation', 'fibonacci')
	));	
}
...
add_action( 'init', 'fibonacci_css' );

function fibonacci_ie_css () {
    echo '<!--[if lt IE 9]>';
    echo '<link rel="stylesheet" href="<?php echo get_template_directory_uri(); ?>/css/ie.css">';
    echo '<![endif]-->';
}
add_action( 'wp_head', 'fibonacci_ie_css' );
...
add_action( 'init', 'fibonacci_scripts' );
...
function fibonacci_entry_meta() {
	echo '<time class="updated" datetime="'. get_the_time('c') .'" pubdate>'. sprintf(__('Posted on %s at %s.', 'fibonacci'), get_the_time('l, F jS, Y'), get_the_time()) .'</time>';
	echo '<p class="byline author vcard">'. __('Written by', 'fibonacci') .' <a href="'. get_author_posts_url(get_the_author_meta('ID')) .'" rel="author" class="fn">'. get_the_author() .'</a></p>';
}
...
class fibonacci_walker extends Walker_Nav_Menu {
...
function fibonacci_active_nav_class( $classes, $item )
{
    if($item->current == 1)
    {
        $classes[] = 'active';
...
add_filter( 'nav_menu_css_class', 'fibonacci_active_nav_class', 10, 2 );
...
function fibonacci_pagination() {
	...
	if ( $paginate_links ) {
		echo '<div class="fibonacci-pagination">';
		echo $paginate_links;
